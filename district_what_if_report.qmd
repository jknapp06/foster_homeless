...
# Technical Assistance and Supports for `r params$student_group` in `r lea_print`

`r intro_sentence` Beginning with dashboard indicators where the `r tolower(params$student_group)` group was assigned a color of red (the lowest performance level), we looked at other student groups with red in that indicator. We then calculated how many students it would take for each group to move from red to orange. That analysis is summarized in the table below.

```{r group_to_orange}
#| echo: FALSE
#| tbl-cap: "Showing how student reporting categories would be affected by improved outcomes"
#| cap-location: bottom
...
if (red_groups > 0) {
  example <- paste0(
    ...
    orange$currdenom[1], " students from the ", orange$student_group_long[1], 
    " group were included in the calculation -- the denominator. The current status is ", orange$currstatus[1], 
    ". The final column indicates the number of students that would need to change ",
    "to earn a color of orange on next year's dashboard if the number of students enrolled remains the same.",
    "\n**This means, if ", abs(orange$students_to_orange[1]), 
    if_else(orange$students_to_orange[1] > 0, " more ", " fewer "), " students** ",
    orange$change[1], ", the **", orange$student_group_long[1], "** student group would be orange ",
    "on next year's dashboard for **", orange$indicator[1], "**.\nAdditionally:"
  )
  
  if (red_groups > 1) {
    bullets <- paste0(
      "* If **", abs(orange$students_to_orange[2:red_groups]),
...
Analysis into the outcome data for `r params$student_group` youth at `r lea_print` reflects that supporting these vulnerable populations may also improve the standing of other groups.
